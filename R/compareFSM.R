...
################################################################################
## to compare to user-defined FSMs  ############################################
################################################################################
# this section compares a user-defined FSM to a decoded GA derived FSM
# the decode() function was created in the main script (ga_fsm.R)

# creating the function this way makes it generalizable to when we have FSMs 
# that may have values in the matrices that arent all simple integers, you can use
# the distance metric that is most appropriate.
compare_fsm <- function(users, gas, comparison = "manhattan"){
        ...
        # "euclidean" does sqrt(sum((x_i - y_i)^2)) - the L2 norm
        # "manhattan" takes abs diff between them - the L1 norm
        # "binary" treats non-zero elements as ‘on’ and zero elements as ‘off’,
