...
There are 30 expeimental groups in the full data set, but for simpicity and to 
keep the computational demands reasonable, we will only analyze 6 of these 
groups.
```{r prepare_data, dependson = "initialize"}
...
```
```{r analyze_data, message=FALSE, dependson = "prepare_data", cache.extra = file.mtime("res.Rds")}
...
```{r plot_results, dependson = "analyze_data"}
...
```{r model_comparison, dependson="analyze_data"}
...
```{r k_fold, message=FALSE, dependson="prepare_data", cache.extra = file.mtime("res2.Rds")}
k_fold_seeds <- sample.int(.Machine$integer.max, length(group_indices)) %>%
  set_names(as.character(group_indices))
if (file.exists("res2.Rds")) {
...
```
```{r k_fold_results, dependson="k_fold"}
